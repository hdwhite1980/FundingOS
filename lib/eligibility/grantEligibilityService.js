...
export class GrantEligibilityService {
  ...
  checkEntityEligibility(userProfile, opportunity) {
    ...
    if (opportunity.source === 'grants_gov' || opportunity.cfda_number) {
      ...
      if (!userProfile.duns_number) {
        requirements.push('Register for DUNS number (now replaced by Unique Entity ID in SAM.gov)')
      }
    }
  ...
  getProfileCompletionRequirements(userProfile, intendedPrograms = []) {
    ...
    if (!userProfile.duns_number) missing.push({
      field: 'duns_number', 
      description: 'DUNS Number (UEI)',
      priority: 'high',
