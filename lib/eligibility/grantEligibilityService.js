...
export class GrantEligibilityService {
  ...
  checkOrganizationType(userProfile, opportunity) {
    ...
    const allowedTypes = opportunity.organization_types || ['all']
    ...
    if (opportunity.small_business_only) {
      ...
      if (!userType) {
        return {
          eligible: true,
          warnings: ['Small business (for-profit) requirement — confirm organization type'],
          requirements: ['For-profit small business eligibility likely required']
        }
      }
      return {
        eligible: false,
    ...
    // If opportunity allows all or doesn't specify types, treat as open
    if (allowedTypes.length === 0 || allowedTypes.includes('all')) {
      if (!userType) {
        return {
          eligible: true,
          warnings: ['Organization type not set — eligibility assumptions may be incomplete']
        }
      }
      return { eligible: true }
    }

    // If user type unknown, don't block — warn instead
    if (!userType) {
      return {
        eligible: true,
        warnings: ['Organization type missing — verify eligibility against funder requirements']
      }
    }
  ...
  checkGeographicEligibility(userProfile, opportunity) {
    ...
    // If we don't have user location, don't hard block — warn
    if (!userState && !userCity) {
      return {
        eligible: true,
        warnings: ['Location not set — verify geographic restrictions']
      }
    }
...
export const enhancedOpportunityService = {
  async getEligibleOpportunities(userProfile, filters = {}) {
    ...
    // Only apply confidence threshold when explicitly filtering to only eligible
