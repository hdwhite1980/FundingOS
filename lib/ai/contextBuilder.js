...
export function classifyAssistantIntent(message) {
  const inputText = message.trim() // Keep original case for case-insensitive regex
  ...
  // DUNS patterns
  if (inputText.match(/\b(duns|d-u-n-s|dun\s*and\s*bradstreet|duns\s*number|d&b|uei|unique\s*entity\s*identifier)\b/i)) {
    return 'duns_lookup'
  }
  
  // EIN/Tax ID patterns
  if (inputText.match(/\b(ein|tax\s*id|employer\s*identification|federal\s*tax)\b/i)) {
    return 'ein_lookup'
  }
  
  // CAGE Code patterns
  if (inputText.match(/\b(cage|cage\s*code|commercial\s*and\s*government\s*entity)\b/i)) {
    return 'cage_lookup'
  }
  
  // SAM registration patterns
  if (inputText.match(/\b(sam|sam\.gov|system\s*for\s*award\s*management|sam\s*registration)\b/i)) {
    return 'sam_lookup'
  }
...
export async function buildDefinitionResponse(context, message, userId) {
  // Use case-insensitive matching for all terminology detection
  if (message.match(/main\s+contact/i)) {
  ...
  if (message.match(/project\s+(manager|director)/i)) {
  ...
  if (message.match(/principal\s+investigator|pi/i)) {
  ...
  if (message.match(/fiscal\s+sponsor/i)) {
  ...
  if (message.match(/matching\s+funds|cost\s+share/i)) {
