...
export function classifyAssistantIntent(userMessage) {
  ...
  // Add EIN/tax ID intent classification
  if (/ein|tax.?id|employer.?id|federal.?id|what.*my.*ein|what.*ein|show.*ein|my.*ein/.test(text)) return 'ein_lookup'
  return 'general'
}
...
export function generateAssistantResponse(intent, context, message) {
  switch (intent) {
    ...
    case 'ein_lookup':
      return buildEINResponse(context, message)
    default:
...
function genericProjectGuidance(context, message) {
  ...
  if (lowerMessage.includes('budget') || lowerMessage.includes('financial') || lowerMessage.includes('cost')) {
    ...
    return `For project "${latest.name}" requesting $${amount.toLocaleString()}, I can help with budget justification, cost categories, and financial sustainability. ${orgName}'s EIN (${companySettings.ein || companySettings.tax_id || 'not set'}) will be needed for most applications. Would you like help structuring your budget narrative or identifying cost-share requirements?`
  }
...
function genericFallback(context, message) {
  ...
  if (/what is (an?|the) ein/i.test(message)) {
    ...
    const ein = profile.ein || profile.tax_id || companySettings.ein || companySettings.tax_id
