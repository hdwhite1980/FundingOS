...
// Enhanced intent classification with comprehensive pattern matching
export function classifyAssistantIntent(message) {
  const lower = message.toLowerCase().trim()
  
  // Campaign-related patterns
  if (lower.match(/\b(campaign|crowdfund|fundrais|donation|supporter|backer|pledge)\b/)) {
    return 'campaign_lookup'
  }
  
  // Project-related patterns
  if (lower.match(/\b(project|initiative|program|activity|task)\b/)) {
  ...
  // DUNS/UEI patterns
  if (lower.match(/\b(duns|uei|unique\s*entity|d\-u\-n\-s)\b/)) {
    return 'duns_lookup'
  }
  
  // CAGE Code patterns
  if (lower.match(/\b(cage|commercial\s*and\s*government|c\-a\-g\-e)\b/)) {
    return 'cage_lookup'
  }
  
  // SAM.gov patterns
  if (lower.match(/\b(sam\.gov|sam\s*registration|system\s*for\s*award)\b/)) {
    return 'sam_lookup'
  }
  
  // Organization info patterns
  if (lower.match(/\b(organization|org|company|business\s*info|profile)\b/)) {
    return 'org_info'
  }
  ...
  // Deadline patterns
  if (lower.match(/\b(deadline|due\s*date|expire|expir|timeline)\b/)) {
...
// Enhanced response generation with comprehensive data integration
...
// General help with comprehensive context sharing
export async function buildGeneralResponse(context, message, userId) {
  try {
    ...
    const campaigns = context?.campaigns || []
    const fundingSummary = context?.funding_summary || {}
    
    let response = `👋 Welcome to your WALI-OS Funding Assistant! `
    
    if (orgProfile?.organization_name) {
      response += `I'm here to help ${orgProfile.organization_name} with all your funding needs.\n\n`
    } else {
      response += `I'm here to help with all your funding needs.\n\n`
    }
    
    // Organization overview
    if (orgProfile) {
      response += `🏢 **Your Organization:**\n`
      response += `• Name: ${orgProfile.organization_name || 'Not set'}\n`
      if (orgProfile.organization_type) response += `• Type: ${orgProfile.organization_type}\n`
      
      // Key identifiers
      const ein = orgProfile.ein || orgProfile.tax_id
      if (ein) response += `• EIN: ${ein}\n`
      
      const duns = orgProfile.duns_number || orgProfile.duns_uei || orgProfile.duns_uei_number
      if (duns) response += `• DUNS/UEI: ${duns}\n`
      
      if (orgProfile.cage_code) response += `• CAGE Code: ${orgProfile.cage_code}\n`
      
      const samStatus = orgProfile.sam_registration || orgProfile.sam_gov_status
      if (samStatus) response += `• SAM.gov Status: ${samStatus}\n`
      
      // Certifications
      const certs = []
      if (orgProfile.minority_owned) certs.push('Minority-Owned')
      if (orgProfile.woman_owned) certs.push('Woman-Owned')
      if (orgProfile.veteran_owned) certs.push('Veteran-Owned')
      if (orgProfile.small_business) certs.push('Small Business')
      if (orgProfile.eight_a_certified) certs.push('8(a)')
      if (orgProfile.hubzone_certified) certs.push('HUBZone')
      
      if (certs.length > 0) {
        response += `• Certifications: ${certs.join(', ')}\n`
      }
    ...
    // Activity overview
    if (projects.length > 0 || applications.length > 0 || campaigns.length > 0 || opportunities.length > 0) {
      response += `📊 **Your Activity:**\n`
      
      if (projects.length > 0) {
        const activeProjects = projects.filter(p => ['active', 'in_progress', 'draft'].includes(p.status))
        response += `• Projects: ${projects.length} total (${activeProjects.length} active)\n`
      }
      
      if (applications.length > 0) {
        const pending = applications.filter(a => ['draft', 'in_progress', 'submitted', 'under_review'].includes(a.status))
        response += `• Applications: ${applications.length} total (${pending.length} pending)\n`
      }
      
      if (campaigns.length > 0) {
        const activeCampaigns = campaigns.filter(c => c.status === 'active')
        response += `• Campaigns: ${campaigns.length} total (${activeCampaigns.length} active)\n`
      }
      ...
      if (opportunities.length > 0) {
        ...
        response += `• Opportunities: ${opportunities.length} total (${activeOpps.length} active)\n`
      }
    ...
    // Funding overview
    if (fundingSummary && Object.keys(fundingSummary).length > 0) {
      response += `� **Funding Overview:**\n`
      
      if (fundingSummary.total_requested > 0) {
        response += `• Total Grant Requests: $${fundingSummary.total_requested.toLocaleString()}\n`
      }
      
      if (fundingSummary.total_project_funding > 0) {
        response += `• Project Funding Needs: $${fundingSummary.total_project_funding.toLocaleString()}\n`
      }
      
      if (fundingSummary.total_campaign_raised > 0) {
        response += `• Crowdfunding Raised: $${fundingSummary.total_campaign_raised.toLocaleString()}\n`
      }
      
      if (fundingSummary.award_rate > 0) {
        response += `• Grant Success Rate: ${fundingSummary.award_rate}%\n`
      }
    ...
    response += `• Answer questions about your projects, applications, and campaigns\n`
    response += `• Help find funding opportunities that match your needs\n`
    response += `• Provide grant writing assistance and budget planning\n`
    response += `• Track deadlines and application status\n`
    response += `• Explain funding terms and requirements\n`
    response += `• Assist with organization profile and certifications\n\n`
    
    response += `Ask me anything like:\n`
    response += `• "Show me my active projects"\n`
    response += `• "What grants are due soon?"\n`
    response += `• "Find opportunities for education projects"\n`
    response += `• "What's my EIN number?"\n`
    response += `• "Help me write a budget for my project"`
    
    return response
    
  } catch (error) {
    console.error('General response error:', error)
    return `👋 I'm your WALI-OS Funding Assistant! I can help you with grant applications, project management, funding opportunities, and organization information. What would you like to know?`
  }
}
