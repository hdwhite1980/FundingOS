...
function formatCampaignsResponse(campaigns, orgName = null) {
  ...
  if (pausedCampaigns.length > 0) {
    response += `**Draft/Paused Campaigns:** ${pausedCampaigns.length}\n\n`
  }
  ...
  response += `\nWant details on a specific campaign? Just ask!`
  ...
  if (activeProjects.length > 0) {
    ...
    activeProjects.slice(0, 5).forEach(project => {
      ...
      if (project.urgency_level === 'high' || project.funding_decision_needed) {
        ...
        response += `  ${project.urgency_level === 'high' ? 'High Priority' : ''}${deadline}\n`
      }
      response += `\n`
    })
  }
...
// Certification lookup - enhanced with privacy check and missing data alerts
