...
export function classifyAssistantIntent(message) {
  ...
  // Compliance patterns
  if (inputText.match(/\b(compliance|audit|regulation|reporting\s+status|policy\s+update|compliance\s+check|compliance\s+score|compliance\s+alerts?)\b/i)) {
...
class DatabaseQueryEngine {
  ...
  ['compliance_overview', this.queryCompliance],
  ...
  async queryCompliance(userId, context) {
    const [tracking, documents, recurring, alerts, preferences, history, analytics] = await Promise.all([
      supabaseAdmin
    ...
    return {
      ...
      alerts: alerts.data || [],
  preferences: preferences.data || (Array.isArray(preferences?.data) ? preferences.data[0] : null),
