...
export class AIEnhancedOpportunityDiscovery {
  ...
  async extractWebContent(url) {
    try {
      // Create a temporary Python script to use trafilatura
      const pythonScript = `
import trafilatura
import sys
import json
...
if __name__ == "__main__":
    ...
    if content:
        ...
        print(json.dumps({"success": False, "content": None}))
`

      const tempScriptPath = join(tmpdir(), `extract_${Date.now()}.py`)
      await writeFile(tempScriptPath, pythonScript)

      try {
        const { stdout } = await this.runPythonScript(tempScriptPath, [url])
        ...
        await unlink(tempScriptPath).catch(() => {}) // Clean up temp file
      }
    } catch (error) {
...
`
  ...
  async storeEnhancedOpportunities(userId, scoredOpportunities) {
    ...
    const opportunitiesToStore = scoredOpportunities
      ...
      .map(opp => ({
        external_id: `ai-discovered-${Date.now()}-${Math.random().toString(36).substring(7)}`,
