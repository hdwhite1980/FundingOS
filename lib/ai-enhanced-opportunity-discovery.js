...
export class AIEnhancedOpportunityDiscovery {
  ...
  async performIntelligentWebSearch(searchQuery, projectType, organizationType) {
    console.log(`🚨 ENTRY: performIntelligentWebSearch called with query: "${searchQuery}", projectType: "${projectType}", orgType: "${organizationType}"`)
    ...
    console.log(`🚨 BASEQUERY CHECK: Generated ${baseQueries ? baseQueries.length : 'undefined/null'} base queries`)
    
    // Target comprehensive funding websites - government, private, corporate, international
    const targetSites = [
      // Government Sources
      'grants.gov',
      'nsf.gov',
      ...
      // Foundation & Philanthropy Databases
      'foundation.org',
      'guidestar.org',
      'candid.org',
      'grantspace.org',
    ...
    // DEBUG: Check if baseQueries is populated
    console.log(`🔍 DEBUG: Generated ${baseQueries.length} base queries for: ${searchQuery}`)
    console.log(`🔍 DEBUG: First 3 queries:`, baseQueries.slice(0, 3))

    // 1. FIRST: Do general web searches (cast wider net)
    console.log(`🔍 Starting general web searches for: ${searchQuery}`)
    
    const generalSearchQueries = baseQueries.slice(0, 10)
    console.log(`🔍 DEBUG: Will run ${generalSearchQueries.length} general searches`)
    
    for (const query of generalSearchQueries) { // Use top 10 most relevant queries for general search
      try {
        console.log(`🌐 Starting general search for: "${query}"`)
        // General web search without site restrictions
        const results = await this.performWebSearch(query)
        searchResults.push(...results)
        console.log(`🌐 General search for "${query}" found ${results.length} results`)
      } catch (error) {
        console.warn(`General search failed for "${query}":`, error.message)
      }
    }

    console.log(`✅ General search phase complete - Found ${searchResults.length} results so far`)

    // 2. THEN: Do site-specific searches (targeted approach)
    console.log(`🔍 Starting site-specific searches for: ${searchQuery}`)
