...
class UnifiedFundingAgent {
  ...
  async initialize() {
    ...
    try {
      ...
      await this.notifyUser('agent_initialized', {
        strategy: this.currentStrategy?.summary || 'Developing funding strategy',
        goals: this.activeGoals?.length || 0,
  ...
  async setAdaptiveGoals(strategy) {
    try {
      // Get user context for goal generation
      const userContext = await this.memory.getUserContext()
      const performanceData = this.performanceMetrics
      
      // Generate adaptive goals using AI service
      const { AIService } = await import('../aiService.js')
      const goalData = await AIService.generateAdaptiveGoals(strategy, userContext, performanceData)
      
      if (goalData && goalData.goals) {
        return goalData.goals
      } else {
        // Fallback to default goals
        return AIService.getDefaultAdaptiveGoals(userContext)
      }
    } catch (error) {
      console.error('Error setting adaptive goals:', error)
      // Return basic fallback goals
      return [
        {
...
OPPORTUNITIES: ${opportunities?.length || 0} tracked opportunities
  ...
  async notifyUser(type, data = {}) {
    try {
      ...
      await supabase.from('agent_notifications').insert([{
        user_id: this.userId,
        type: type,
        title: this.generateNotificationTitle(type, safeData),
        message: this.generateNotificationMessage(type, safeData),
        data: safeData,
        created_at: new Date().toISOString()
      }])
      
      console.log(`📢 Notification sent: ${type} for user ${this.userId}`)
    } catch (error) {
      console.error('Notification error:', error)
      // Don't rethrow - notification failures shouldn't crash the agent
    }
  }
  ...
  generateNotificationTitle(type, data) {
    const titles = {
      ...
      'agent_paused': '⏸️ Agent Paused',
      'agent_resumed': '▶️ Agent Resumed',
      'agent_stopped': '⏹️ Agent Stopped'
    }
  ...
  generateNotificationMessage(type, data) {
    const messages = {
      'agent_initialized': `I'm now actively managing your funding strategy with ${data?.goals || 0} goals focused on ${data?.strategy || 'your objectives'}.`,
      'strategy_updated': `I've adapted your funding strategy based on recent performance: ${data?.reasoning || 'Continuous optimization'}`,
      'critical_deadline': `${data?.opportunity || 'An opportunity'} has only ${data?.daysRemaining || 'few'} day(s) remaining. ${data?.recommendation || 'Please review immediately.'}`,
      'high_priority_decision': `New recommendation available for your review regarding ${data?.opportunity || 'your funding strategy'}.`,
      'performance_report': `Weekly performance: ${data?.metrics?.opportunitiesDiscovered || 0} opportunities discovered, ${data?.metrics?.applicationsGenerated || 0} applications generated.`,
      'agent_paused': 'Your AI funding agent has been paused and will resume monitoring when reactivated.',
      'agent_resumed': 'Your AI funding agent has resumed monitoring and active management.',
      'agent_stopped': 'Your AI funding agent has been stopped. Reactivate when you need active funding management.'
    }
  ...
  "confidence": 0.0-1.0
    ...
    try {
      ...
      return {
        position: 'moderate',
        urgency: 'medium',
        opportunities: ['Explore new funding sources', 'Diversify application portfolio'],
        threats: ['Application deadlines', 'Competitive funding landscape'],
        priorities: ['Opportunity discovery', 'Application preparation'],
        recommendations: ['Set up systematic opportunity monitoring', 'Improve application quality'],
        confidence: 0.5
      }
    }
  }
