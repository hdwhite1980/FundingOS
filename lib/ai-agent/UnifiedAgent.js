...
class UnifiedFundingAgent {
  ...
  async handleGeneralQuery(message, context) {
    // Check if this might be an opportunity-related query that wasn't caught
    const lowerMessage = message.toLowerCase()
    ...
    // Check for web discovery requests
    if (lowerMessage.includes('search internet') || lowerMessage.includes('find new') || lowerMessage.includes('discover opportunities') || lowerMessage.includes('web search')) {
  ...
  async handleWebDiscoveryRequest(message, context) {
    try {
      ...
      // Call the AI discovery API
      const discoveryResponse = await fetch(`${this.apiBaseUrl.replace('/unified-agent', '')}/discover-opportunities`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  extractSearchQuery(message) {
    ...
    const cleanMessage = message
      .replace(/search internet for|find new|discover opportunities|web search for|look for/gi, '')
      .replace(/funding|opportunities|grants/gi, '')
      .trim()
    
    // If nothing meaningful remains, return the original message
    return cleanMessage.length > 3 ? cleanMessage : message
  }
