...
class UnifiedFundingAgent {
  ...
  async handleWebDiscoveryRequest(message, context) {
    try {
      ...
      // Import and use the discovery system directly (no HTTP requests needed)
      const { AIEnhancedOpportunityDiscovery } = await import('../ai-enhanced-opportunity-discovery.js')
      const { supabase } = await import('../supabase.js')
      const discoverySystem = new AIEnhancedOpportunityDiscovery(supabase)

      // Perform comprehensive opportunity discovery and analysis directly
      const discoveryResults = await discoverySystem.discoverAndAnalyzeOpportunities({
