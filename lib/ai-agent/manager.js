...
class AgentManager {
  ...
  async startAgent(userId) {
    ...
    try {
      const agent = new AIFundingAgent(userId, openai, supabase)
  ...
  async startAllAgents() {
    ...
    try {
      ...
      const { data: users, error } = await supabase
        ...
        // Commented out agent_enabled filter since column may not exist yet
  ...
  async processEmailQueue() {
    try {
      ...
      // Update metrics
      this.performanceMetrics.emailsSent += emailsProcessed || 0
