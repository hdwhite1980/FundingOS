...
export function validateSupabaseConfig() {
  ...
  const publicEnv = (win && win.__PUBLIC_ENV__) ? win.__PUBLIC_ENV__ : undefined
  const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL || publicEnv?.NEXT_PUBLIC_SUPABASE_URL
  const supabaseAnonKey = process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY || publicEnv?.NEXT_PUBLIC_SUPABASE_ANON_KEY
  ...
  // Only validate production values at runtime, not during build
