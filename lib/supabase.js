...
export const angelInvestorServices = {
  ...
  async getOrCreateAngelInvestor(userId, userEmail) {
    try {
      ...
      if (getError && getError.code === 'PGRST116') {
        ...
        const { data: createdInvestor, error: createError } = await supabase
          .from('angel_investors')
          .insert([newInvestor])
          .select()
          .single()
  ...
  async updateInvestmentPreferences(userId, partialPreferences) {
    try {
      ...
      const { data, error } = await supabase
        .from('angel_investors')
        .update({ investment_preferences: merged, updated_at: new Date().toISOString() })
        .eq('user_id', userId)
        .select()
        .single()
  ...
  async getAngelInvestorStats(userId) {
    try {
      const [investorData, portfolioData] = await Promise.all([
        supabase
          .from('angel_investors')
          .select('total_invested, portfolio_value, active_investments')
          .eq('user_id', userId)
          .single(),
        supabase
          .from('angel_investments')
          ...
          `)
          .eq('angel_investors.user_id', userId)
          .eq('status', 'active')
      ])

      const investor = investorData.data || { total_invested: 0, portfolio_value: 0, active_investments: 0 }
...
export const companiesServices = {
  ...
  async listCompanyOverviews(userId, filters = {}) {
    try {
      ...
      let query = supabase.from('company_portfolio_overview').select('*', { count: 'exact' }).eq('user_id', userId);
