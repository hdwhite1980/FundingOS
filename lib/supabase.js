    ...
    async getOpportunities(filters = {}) {
      try {
        ...
        if ((data?.length || 0) === 0 && filters.projectTypes && filters.projectTypes.length > 0) {
          ...
          // Apply other filters (but not projectTypes)
          if (filters.organizationType) {
  ...
  investors: {
    ...
    async createInvestor(userId, investorData) {
      try {
        ...
        return await this.createInvestorViaAPI(userId, investorData)
      } catch (error) {
    ...
    async deleteInvestor(userId, investorId) {
      try {
        ...
        return await this.deleteInvestorViaAPI(userId, investorId)
      } catch (error) {
