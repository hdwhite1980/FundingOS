      ...
      try {
        ...
        if (filters.projectTypes && filters.projectTypes.length > 0) {
          ...
          const projectTypeMapping = {
            ...
            'technology_implementation': ['technology', 'contract', 'services', 'research', 'implementation'], // Added mapping
        ...
        if (excludeResourcesByDefault) {
          ...
          // Exclude items tagged with resource-related categories
          try {
            query = query.not('ai_categories', 'ov', resourceTags)
          } catch (e) {
            console.warn('Array overlap exclusion not supported, relying on AI flag only:', e?.message)
          }
  ...
  const { data, error } = await query.limit(200)
        ...
        if ((data?.length || 0) === 0 && filters.projectTypes && filters.projectTypes.length > 0) {
          ...
          if (excludeResourcesByDefault) {
            ...
            try {
              fallbackQuery = fallbackQuery.not('ai_categories', 'ov', resourceTags)
            } catch (e) {
              console.warn('Fallback array overlap exclusion not supported:', e?.message)
            }
            const resourceTitleCues = [
              '%ad grant%', '%ad grants%', '%nonprofit advertising%', '%promotional credit%', '%cloud credit%'
            ]
  ...
  async getOpportunities(filters = {}) {
    try {
      ...
      if (excludeResourcesByDefault) {
        ...
        query = query.neq('ai_analysis->>isNonMonetaryResource', 'true')
        // And exclude items tagged with resource categories
        try {
          query = query.not('ai_categories', 'ov', resourceTags)
        } catch {
          // Some clients may not support 'ov' operator; ignore
        }
      }
