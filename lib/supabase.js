...
export const directUserServices = {
    ...
    async getOpportunities(filters = {}) {
      try {
        ...
        if (filters.nonMonetaryOnly) {
          ...
          // Relaxed: only exclude obvious monetary grant programs by title
          query = query
  ...
  // Resource-identifying tags (comprehensive set covering all non-monetary resource types)
  const resourceTags = [
    'resources',
    'non_monetary',
    'in_kind',
    ...
    'accelerator'
        ...
        if (excludeResourcesByDefault) {
          ...
          // NOTE: Skipping AI analysis/category filters here because NULL-safe filtering in PostgREST is complex
          // and would exclude unanalyzed Grants.gov opportunities. Client-side filters will catch resources.
          // Only apply heuristic guardrails for well-known resource cues
  ...
  const { data, error } = await query.limit(200)
        ...
        if ((data?.length || 0) === 0 && filters.projectTypes && filters.projectTypes.length > 0) {
          ...
          if (excludeResourcesByDefault) {
            ...
            // Skip AI filters to allow unanalyzed Grants.gov items; rely on heuristics
            const resourceTitleCues = [
              '%ad grant%', '%ad grants%', '%nonprofit advertising%', '%promotional credit%', '%cloud credit%'
            ]
            resourceTitleCues.forEach(cue => {
...
export const opportunityService = {
  async getOpportunities(filters = {}) {
    try {
      ...
      // PROJECT TYPES: Include "general" for Grants.gov opportunities
      if (filters.projectTypes && filters.projectTypes.length > 0) {
        const projectType = filters.projectTypes[0]
        const inclusiveTypes = [...filters.projectTypes]
        
        // Map user project types to include relevant types + "general" for Grants.gov
        const projectTypeMapping = {
          'technology': ['technology', 'contract', 'services', 'research', 'general'],
          'technology_implementation': ['technology', 'contract', 'services', 'research', 'implementation', 'general'],
          'commercial_development': ['commercial_development', 'contract', 'services', 'development', 'general'],
          'infrastructure': ['infrastructure', 'contract', 'services', 'construction', 'general'],
          'research': ['research', 'contract', 'services', 'technology', 'general'],
          'healthcare': ['healthcare', 'contract', 'services', 'medical', 'general'],
    ...
    'accelerator'
      ...
      if (filters.nonMonetaryOnly) {
        query = query
          ...
          .is('amount_max', null)
          .eq('ai_analysis->>isNonMonetaryResource', 'true')
        // Relaxed: removed description filters that were too strict
        // (e.g., Google Ad Grants mentions "$10,000 worth of ads" but is still non-monetary)
        // Only exclude obvious monetary grant titles
          .not('title', 'ilike', '%nofo%')
          .not('title', 'ilike', '%sbir%')
          .not('title', 'ilike', '%sttr%')
      }
