  ...
  applications: {
    ...
    async getSubmissionStats() {
      try {
        ...
        if (!session?.user) {
          console.warn('getSubmissionStats: No authenticated session')
          return {
            totalSubmissions: 0,
            totalRequested: 0,
            totalAwarded: 0,
    ...
    async getSubmissions(filters = {}) {
      try {
        const { data: { session } } = await supabase.auth.getSession()
        if (!session?.user) {
          console.warn('getSubmissions: No authenticated session')
          return []
        }
...
export const applicationProgressService = {
  async getSubmissionStats() {
    try {
      const { data: { session } } = await supabase.auth.getSession()
      if (!session?.user) {
        console.warn('applicationProgressService getSubmissionStats: No authenticated session')
        return {
          totalSubmissions: 0,
          totalRequested: 0,
          totalAwarded: 0,
  ...
  async getSubmissions(filters = {}) {
    try {
      const { data: { session } } = await supabase.auth.getSession()
      if (!session?.user) {
        console.warn('applicationProgressService getSubmissions: No authenticated session')
        return []
      }
