  ...
  opportunities: {
    async getOpportunities(filters = {}) {
      try {
        ...
        // Non-monetary only: additionally exclude records that specify cash amounts
        if (filters.nonMonetaryOnly) {
          console.log('🧲 Applying non-monetary only filter (no cash amounts or variable monetary wording)')
          // Exclude if numeric amounts are present
          query = query.is('amount_min', null).is('amount_max', null)
          // Additional guard: exclude descriptions that explicitly mention stipend/grant $$ while leaving resource programs
          // Note: This uses ilike NOT patterns to reduce obvious monetary posts sneaking in without amounts
          query = query
            .not('description', 'ilike', '%$%')
            .not('description', 'ilike', '% grant of %')
            .not('description', 'ilike', '% cash %')
            .not('description', 'ilike', '% award of %')
        }
