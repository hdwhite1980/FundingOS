    ...
    async getOpportunities(filters = {}) {
      try {
        ...
        if (filters.organizationType) {
          query = query.contains('eligible_applicants', [filters.organizationType])
        }

        if (filters.minAmount) {
          query = query.gte('award_ceiling', filters.minAmount)
        }

        if (filters.maxAmount) {
          query = query.lte('award_floor', filters.maxAmount)
        }
  ...
  donors: {
    ...
    async getDonorStats(userId) {
      try {
        const { data, error } = await supabase
          .from('donors')
          .select('donation_amount, is_major_donor')
          .eq('user_id', userId)
        ...
        const totalRaised = donors.reduce((sum, donor) => sum + (donor.donation_amount || 0), 0)
        ...
        let query = supabase
          ...
          .select(`
            *,
            ...
            opportunities:opportunity_id (title, agency_name)
          `)
          .eq('user_id', userId)
          .order('created_at', { ascending: false })
    ...
    async addProjectOpportunity(userId, projectId, opportunityId, matchData = {}) {
      try {
        const newMatch = {
          ...
          ai_fit_score: matchData.aiScore || 0,
          status: 'identified',
          created_at: new Date().toISOString(),
          updated_at: new Date().toISOString(),
          ...matchData
