  ...
  projects: {
    ...
    async createProject(userId, projectData) {
      try {
        ...
        // If project_categories is an array, convert it to JSON string for database storage
        if (Array.isArray(newProject.project_categories)) {
          newProject.project_categories = JSON.stringify(newProject.project_categories)
        }

        // Ensure primary_goals is properly handled as an array
        if (Array.isArray(newProject.primary_goals)) {
          newProject.primary_goals = JSON.stringify(newProject.primary_goals)
        }

        // Ensure preferred_funding_types is properly handled as an array  
        if (Array.isArray(newProject.preferred_funding_types)) {
          newProject.preferred_funding_types = JSON.stringify(newProject.preferred_funding_types)
        }
  ...
  projectOpportunities: {
    ...
    async addProjectOpportunity(userId, projectId, opportunityId, matchData = {}) {
      try {
        ...
        const { default: scoringCache } = await import('./scoringCache.js')
        
        // Calculate and cache the score automatically
        const scoreResult = await scoringCache.getOrCalculateScore(userId, projectId, opportunityId)
