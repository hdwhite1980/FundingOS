...
-- 6. Helper Function (create before policies)
...
-- 7. Create RLS Policies (with existence checks)
DO $$ 
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE policyname = 'Users can view their own cached forms' 
    AND tablename = 'form_analysis_cache'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE policyname = 'Users can create form analysis cache' 
    AND tablename = 'form_analysis_cache'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE policyname = 'Users can update their cached forms' 
    AND tablename = 'form_analysis_cache'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE policyname = 'Users can manage their AI sessions' 
    AND tablename = 'form_ai_sessions'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE policyname = 'Users can access messages from their sessions' 
    AND tablename = 'form_ai_messages'
  ) THEN
